...
\def\manual{\htmladdnormallink {man.tex}	  {../man/man.html}}
\def\refman{\htmladdnormallink {reference manual} {../man/man.html}}
...
@mDate@. For a documentation of the available commands you should read
the \refman{} (\file {man.tex} or one of its derivatives \file
{man.dvi}, \file {man.ps}).
...
This package depends in its operation on quite a few others. Here is a
list of them:
...
\item	\strong {\tcl}, of course. The package will compile with 7.6
	...
	\strong {source distribution} is required. Instructions on how
...
\item	\strong {Memchan}. Trf can be used without, but the
	...
	source 	       & \url {ftp://ftp.westend.com/pub/aku/memchan1.3.tar.gz} \\ \hline
	source (win)   & \url {ftp://ftp.westend.com/pub/aku/memchan13.zip} \\ \hline
	binary (win32) & \url {ftp://ftp.westend.com/pub/aku/mchan13b.zip} \\ \hline
	\end {tabular}}
	\html{	\html{
	\begin{itemize}
	\item	\htmladdnormallink {source}{ftp://ftp.westend.com/pub/aku/memchan1.3.tar.gz}
	\item	\htmladdnormallink {source (win)}{ftp://ftp.westend.com/pub/aku/memchan13.zip}
	\item	\htmladdnormallink {binary (win32)}{ftp://ftp.westend.com/pub/aku/mchan13b.zip}
	\end{itemize}}}
...
\item	Apply the 2 patches \file {standard.patch} and \file
	...
	\strong {Danger}: The working directories required by the
	...
	available. Tcl 8.0b1 on the other hand has \cmd {fcopy} and
...
\html{Additional sources of information are \file {rfc1950.txt}, \file
{rfc1951.txt} and \file {rfc1952.txt}.}
\latex{The official homepage can be found at \url
{http://www.cdrom.com/pub/infozip/zlib/}. A downloading page is
available there too. Additional sources of information are \file
{rfc1950.txt}, \file {rfc1951.txt} and \file {rfc1952.txt}.}
...
\latex {The official FAQ page is \url{\ssleaypage}.}
\html  {A \ssleayfaq{} is available.}
...
\item	In contrast to earlier versions \strong {the system will build
	a shared library by default}. Give option \opt
	{--disable-shared} to \cmd {configure} if this is not
	desired.
...
\item	With \opt{--enable-binio} you are able to activate compilation
	and linkage of command \cmd {binio}.
\end {itemize}
